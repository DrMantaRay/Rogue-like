...
//Graphics
int battle(monster p, hero h,int movefirst, list yourmovelist, moves movelist, WINDOW*topwin) {
  wclear(topwin);
  destroy_win(topwin);
  ...
  while (1) {
	...
	mvprintw(topwin,1,1,"The sneaky %s dodged your attack!", p->name);
	}
      wrefresh(topwin);
      wgetch(topwin);
	} else if (damage > -1) {
